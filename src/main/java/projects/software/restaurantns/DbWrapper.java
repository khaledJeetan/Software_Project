...
public class DbWrapper {
    ...
    public static List<RestaurantService> getRestaurants(String serv){
        ...
        sql = """
                ...
                r.cover_photo,service.name service,restaurant_service.price,r.OPENING_HOURS from restaurant r
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                restaurant.setOpeningHours(resultSet.getString("opening_hours"));
    ...
    public static List<Restaurant> getAllRestaurants(){
        try {
            ...
            sql = """
                select r.id,r.name,Address.city,Address.location,r.has_Delivery,r.phone,r.is_health_approved,
                r.cover_photo,r.OPENING_HOURS from restaurant r
                inner join address on address.id = r.address """;
            con = main.getConnection();
            stmt = con.prepareStatement(sql);
            ...
            while(rs.next()){
                ...
                restaurant.setHasDelivery(rs.getBoolean("has_delivery"));
                restaurant.setPhone(rs.getInt("phone"));
                restaurant.setHealthApproved(rs.getBoolean("is_health_approved"));
                restaurant.setCoverPhoto(rs.getBinaryStream("cover_photo"));
    ...
    public static boolean updateUser(User user) {
        sql = "update user_tb set password = ?, UPDATED_AT = ? where username = ?";
        try {
            con = main.getConnection();
            stmt = con.prepareStatement(sql);
    ...
    public static boolean deleteRestaurant(Restaurant restaurant){
        try {
            ...
            System.out.println("before Query Deleting Restaurant");
            sql = "DELETE FROM RESTAURANT WHERE ID  = ?";
            con = main.getConnection();
            stmt = con.prepareStatement(sql);
            stmt.setInt(1, restaurant.getId());
            stmt.executeUpdate();
            System.out.println("after Query Deleting Restaurant");
